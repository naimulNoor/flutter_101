...
class _NewProductPageState extends State<NewProductPage> {
  ...
  void _takeGalleryPicture() {
    ImagePicker().getImage(
        source: ImageSource.gallery, maxWidth: 100.0, maxHeight: 100.0).then((
        value) {
      print(value.path);
      setState(() {
        imgPath = value.path;
  ...
  void _takeGalleryPicture_withoutThen() async {
    PickedFile pickedFile = await ImagePicker().getImage(
        source: ImageSource.gallery, maxWidth: 100.0, maxHeight: 100.0);
    setState(() {
  ...
  void _saveProduct() {
    if (formkey.currentState.validate()) {
      ...
      DBSQLITE.insert(product).then((value) => {
      if(value>0){
